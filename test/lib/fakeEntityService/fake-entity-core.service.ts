...
export abstract class FakeEntityCoreService<TEntity> {
  ...
  */
   ...
   */
  protected entityPostprocessor: (
    entity: TEntity,
    index: number,
  ) => TEntity | Promise<TEntity>;

  protected getFakeFields(customFields?: Partial<TEntity>): Partial<TEntity> {
  ...
  */
  ...
  protected async sequentialResolver(
  ...
  protected async postprocessEntities(entities: TEntity[]): Promise<TEntity[]> {
    if (typeof this.entityPostprocessor === 'function') {
      const postprocessingEntitiesPromises = entities.map((entity, i) =>
        this.entityPostprocessor(entity, i),
      );
  ...
  */
  public addStates(
    ...
    if (Array.isArray(states)) {
      const statesArray: Partial<TEntity>[] = states.map((state) =>
  ...
  public addFieldSequence<K extends keyof TEntity>(
    field: K,
    values: TEntity[K][],
  ): this {
    this.addStatesGenerator(
      values.map((value) => {
  ...
  public abstract create(customFields?: Partial<TEntity>): Promise<TEntity>;
